...
int main(void)
{
    ...
    while (1)
    {
        ...
        //온습도
        static uint32_t humid_counter = 0;
        static uint32_t hum = 0;
        static uint32_t temp = 0;

        humid_counter++;
        if (humid_counter >= 20)   // 루프가 100ms 딜레이 기준 → 약 2초
        {
            humid_counter = 0;
            ...
            if (!success)
            {
                // 체크섬 오류 → 메시지만 띄우고 이전 값 유지
                lcd_set_cursor(0, 0);
                lcd_print_string("Humidity ERR   ");
                lcd_set_cursor(1, 0);
                lcd_print_string("Temp ERR       ");
            }
        }

        /*********************************************
         * LCD에 온습도 표시 (매 루프 표시 OK)
         *********************************************/
